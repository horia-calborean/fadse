...
The hypervolume.csv file contains two columns. The first column is the hypervolume of the current generation (first line = generation 1, second line = generation 2, etc.). The second column is the hypervolume of all the individuals produced up to that point (an core.algorithm might loose good individuals during the DSE process, thus the values from the first column might decrease sometimes) - this value should always be equal or larger than the one before. We recommend to use the values from the first column since they provide a better image about the convergence of the core.algorithm. If you want to show the best possible hypervolume with everything the core.algorithm has found use the second column. 
