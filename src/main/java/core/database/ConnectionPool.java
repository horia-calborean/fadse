...
public class ConnectionPool {
    ...
    /** Max available number of DB connections */
    private static final int MAX_AVAILABLE = 25;

    /**
     * @param inputData the InputData to set
     */
    public static void setInputDocument(InputData inputData) {
        // if the connection information in the inputDoc has changed, close all connections.
        HashMap<String, Object> databaseInfo = (HashMap<String, Object>) inputData.get(CommonSetupParameters.DATABASE);

        String my_url = "jdbc:mysql://" +  databaseInfo.get("ip")
                + ":" + databaseInfo.get("port")
                + "/" + databaseInfo.get("name");
        String my_user = (String) databaseInfo.get("user");
        String my_passwd = (String) databaseInfo.get("password");
...
//            log.info("Connection information has been changed to " + url + " " + user + " " + passwd);
    ...
    /** Array containing the DB connections */
    protected Connection[] items = new Connection[MAX_AVAILABLE];
    /** Array with flags saying if the connection is actively used */
    ...
    public Connection getItem() throws InterruptedException, SQLException {
        log.debug("Grabbing a connection from the pool...");
        Connection result = null;

        for (int i = 0; i < 5 && result == null; i++) {
            result = getUncheckedItem();
            if (!connectionIsValid(result)) {
                log.warn("Connection shall be fetched, " + i + "th try was not successful.");
                putItem(result);
                result = null;
            }
        }
    ...
    private boolean connectionIsValid(Connection dbConn) {
        ...
        try {
            ...
            try {
                ...
                log.error("Exception during closing of the test statement: ", ex);
            }
        }
...
//        log.info("AFTER  More DB-Connections are available: " + available.availablePermits());
    ...
    protected synchronized Connection getNextAvailableItem() throws SQLException {
        for (int i = 0; i < MAX_AVAILABLE; ++i) {
            if (i == 22) {
                log.debug("TWENTYTWO - CRITICAL!!");
            }

            if (i == 20) {
                log.debug("TWENTY - CRITICAL!!");
            }

            if (!used[i]) {
                used[i] = true;
                if (log.isDebugEnabled()) {
                    log.debug(i + " Connection is occupied");
                }
                return getItem(i);
            }
        }
