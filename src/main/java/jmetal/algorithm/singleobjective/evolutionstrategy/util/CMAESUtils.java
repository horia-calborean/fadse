package jmetal.algorithm.singleobjective.evolutionstrategy.util;
      ...
      for (j = 0; j < n; ++j) {
        ...
        if (Math.abs(cc - c[biggerValue(i,j)][smallerValue(i,j)])
          ...
          JMetalLogger.logger.severe(
              "CMAESUtils.checkEigenSystem: WARNING - imprecise jmetal.experiment output detected " + s);
          ++res;
        }
        if (Math.abs(dd - (i == j ? 1 : 0)) > 1e-10) {
          s = i + " " + j + " " + dd;
          JMetalLogger.logger.severe("CMAESUtils.checkEigenSystem():" +
                  " WARNING - imprecise jmetal.experiment output detected (Q not orthog.) " + s);
          ++res;
        }
      }
    }
