package jmetal.core.util.legacy.qualityindicator.impl.hypervolume.impl;
...
import jmetal.core.util.legacy.front.util.FrontNormalizer;
...
import jmetal.core.util.solutionattribute.impl.HypervolumeContributionAttribute;
...
public class WFGHypervolume<S extends Solution<?>> extends Hypervolume<S> {
   ...
   * @throws FileNotFoundException
   */
  public WFGHypervolume(jmetal.core.util.legacy.front.Front referenceParetoFront) {
   ...
   */
  private double hypervolume(jmetal.core.util.legacy.front.Front front, jmetal.core.util.legacy.front.Front referenceFront) {

    jmetal.core.util.legacy.front.Front invertedFront;
  ...
  public List<S> computeHypervolumeContribution(List<S> solutionList, List<S> referenceFrontList) {
    if (solutionList.size() > 1) {
      jmetal.core.util.legacy.front.Front front = new ArrayFront(solutionList);
      jmetal.core.util.legacy.front.Front referenceFront = new ArrayFront(referenceFrontList);
      ...
      jmetal.core.util.legacy.front.Front normalizedFront = frontNormalizer.normalize(front);
      ...
      jmetal.core.util.legacy.front.Front invertedFront = FrontUtils.getInvertedFront(normalizedFront);

      // shift away from origin, so that boundary points also get a contribution > 0
      for (int i = 0; i < invertedFront.getNumberOfPoints(); i++) {
        jmetal.core.util.point.Point point = invertedFront.getPoint(i);
